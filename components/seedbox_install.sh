...
show_update_error_() {
	if [ -s /tmp/seedbox_update_error ]; then
		warn "Last apt error output:"
		tail -n 20 /tmp/seedbox_update_error >&2
	fi
}
seperator() {
	echo -e "\n"
...
run_step_() {
	...
	shift
	info_2 "$description"
	"$@" >/dev/null 2>>/tmp/seedbox_update_error
	if [ $? -ne 0 ]; then
		fail_3 "$description: FAIL"
		echo
		show_update_error_
		return 1
	fi
	info_3 "$description: Successful"
	echo
	return 0
}

update() {
	: >/tmp/seedbox_update_error

	run_step_ "Updating package index" apt-get -qqy update || fail_exit "System update failed. Check /tmp/seedbox_update_error"

    # Install Dependencies
	if ! command -v sudo >/dev/null 2>&1; then
		run_step_ "Installing sudo" apt-get install sudo -qqy || fail_exit "Sudo Installation Failed. Check /tmp/seedbox_update_error"
	else
		boring_text "sudo already installed"
	fi
	if ! command -v wget >/dev/null 2>&1; then
		run_step_ "Installing wget" apt-get install wget -qqy || fail_exit "Wget Installation Failed. Check /tmp/seedbox_update_error"
	else
		boring_text "wget already installed"
	fi
	if ! command -v curl >/dev/null 2>&1; then
		run_step_ "Installing curl" apt-get install curl -qqy || fail_exit "Curl Installation Failed. Check /tmp/seedbox_update_error"
	else
		boring_text "curl already installed"
	fi
	if ! command -v killall >/dev/null 2>&1; then
		run_step_ "Installing psmisc" apt-get install psmisc -qqy || fail_exit "Psmisc Installation Failed. Check /tmp/seedbox_update_error"
	else
		boring_text "psmisc already installed"
	fi
	return 0
}
...
install_autobrr_() {
	...
	if ! getent passwd "$username" >/dev/null; then	# return if username does not exist
...
install_vertex_() {
	...
	if [[ -z $vertex_port ]]; then
		warn "Vertex port not set, using default port 3000"
		vertex_port=3000
	fi
	...
	if ! command -v apparmor_parser >/dev/null 2>&1; then
		warn "AppArmor is not available; continuing without installing it"
	fi
...
install_autoremove-torrents_() {
	...
	if command -v pipx >/dev/null 2>&1; then
		su "$username" -s /bin/sh -c "pipx list | grep -q 'package autoremove-torrents' || pipx install autoremove-torrents"
		# Check if installation fail
		if [ $? -ne 0 ]; then
			fail "Autoremove-torrents installation failed"
			return 1
...
kernel_settings_() {
	...
	cat << EOF >/etc/sysctl.d/99-seedbox.conf
